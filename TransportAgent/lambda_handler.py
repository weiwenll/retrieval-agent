"""
Lambda handler for TransportAgent with S3 input/output and async support

Expected API Gateway payload:
{
    "bucket_name": "iss-travel-planner",
    "key": "transport_agent/20251101120000.json",
...
- key: Full S3 key path to input file (e.g., "transport_agent/20251101120000.json")
...
Output location: s3://{bucket_name}/transport_agent/processed/{filename}.json
Status location (async): s3://{bucket_name}/transport_agent/status/{session_id}.json
"""
...
# Output prefix for transport agent output (goes into processed folder)
OUTPUT_PREFIX = 'transport_agent/processed/'
STATUS_PREFIX = 'transport_agent/status/'
...
def lambda_handler(event, context):
    ...
    try:
        ...
        log_structured('INFO', 'Request received',
            ...
            workflow_chain=f"{sender_agent} -> TransportAgent")
        ...
        try:
            ...
            log_structured('INFO', 'Starting transport processing',
                session_id=session_id,
            ...
            total_routes = sum(len(day_data.get('connections', [])) for day_data in transport_data.values())

            log_structured('INFO', 'Transport processing completed',
                session_id=session_id,
                stage='transport_processing',
            ...
            # Prepare response - RETURN ONLY TRANSPORT OBJECT
            response_body = {
                'status': 'success',
                ...
                'transport': transport_data,  # <-- RETURN TRANSPORT OBJECT WITH DATES
                'summary': {
                    'days_processed': len(transport_data),
                    'total_routes': total_routes,
            ...
            if async_mode:
                ...
                return {
                    ...
                    'body': json.dumps({
                        ...
                        'transport': transport_data,  # Return transport object
                        'summary': {
                            'days_processed': len(transport_data),
                            'total_routes': total_routes,
                            'processing_time_seconds': result.get('processing_time_seconds', 0)
            ...
            log_structured('INFO', 'Request completed successfully',
                ...
                total_routes=total_routes,
                success=True,
                workflow_chain=f"{sender_agent} -> TransportAgent -> Complete")
