"""
SQS-triggered Lambda handler for processing transport tasks asynchronously.
...
def lambda_handler(event, context):
    """
    Process transport tasks from SQS queue.
    ...
    for record in event['Records']:
        try:
            ...
            # Process the transport task
            result = process_transport_task(
...
def process_transport_task(bucket_name: str, input_key: str, session_id: str, task_id: str, filename: str):
    ...
    try:
        ...
        log_structured('INFO', 'Transport processing completed',
            session_id=session_id,
            stage='transport_processing',
            duration_ms=round(transport_duration * 1000, 2))
